import { Joi, ValidationNew } from './Validation'
...
export const FunctionResult = Joi.object({
  ...
  Status: Joi.string().valid(...[FunctionResultStatus.Success, FunctionResultStatus.Failure]),
  ReturnValue: Joi.any(),
  ExtraData: Joi.any(),
  ...
  if (v.Status === FunctionResultStatus.Failure) {
    if (v.Messages.length === 0) {
      throw Error('If status is Failure, you must add something to the Messages property')
    }
  }
  return v
})

FunctionResult.new = ValidationNew(FunctionResult,
  {
  ...
  undefined,
  {
    AddMessageError: function (message, section) {
      this.Messages.push({ Message: message, Type: FunctionResultMessageTypes.Error, Section: section })
    ...
    AddMessageWarning: function (message, section) {
      this.Messages.push({ Message: message, Type: FunctionResultMessageTypes.Warning, Section: section })
    },

    AddMessageInfo: function (message, section) {
      this.Messages.push({ Message: message, Type: FunctionResultMessageTypes.Info, Section: section })
    },

    AddMessageRule: function (message, field, section) {
      this.Messages.push({ Message: message, Type: FunctionResultMessageTypes.Rule, Field: field, Section: section })
      this.Status = FunctionResultStatus.Failure
    },

    AddMessageAuthorization: function (message) {
      this.Messages.push({ Message: message, Type: FunctionResultMessageTypes.Authorization })
      this.Status = FunctionResultStatus.Failure
    },

    AddMessageAuthentication: function (message) {
      this.Messages.push({ Message: message, Type: FunctionResultMessageTypes.Authentication })
      this.Status = FunctionResultStatus.Failure
    },

    AddMessageModelFieldTypeError: function (message) {
      this.Messages.push({ Message: message, Type: FunctionResultMessageTypes.ModelFieldTypeError })
    }
  })
