...
import { DesktopDateTimePicker, MobileDateTimePicker, LocalizationProvider } from '@material-ui/pickers'
...
export class DateTimeBoxClass extends React.Component {
  onChange = (e) => {
  ...
  render () {
    const { displayMode = 'desktop', displayFormat = 'MM/DD/YYYY hh:mm a', ...rest } = this.props
    return (
        ...
        <FieldLayoutBox {...rest}>
          {({ showError, value, onChange, name, placeHolder, loading, readonly, testId, label }) => {
            const DateComponent = readonly === true
              ? FilledInput
              : displayMode === 'desktop' ? DesktopDateTimePicker : MobileDateTimePicker
...
export const DateTimeBox = ({ parse, format, fieldType, ...rest }) => { return <Field parse={fieldType ? FieldValueParser(fieldType) : parse} format={fieldType ? FieldValueFormatter(fieldType) : format} {...rest} component={DateTimeBoxClass} /> } // eslint-disable-line
